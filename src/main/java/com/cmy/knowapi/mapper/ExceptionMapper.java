...
public interface ExceptionMapper extends MyMapper<SystemException> {
    ...
    @Select("SELECT e.id as eid,e.code as exceptionCode,e.flow_code as flowType,e.exception_type as exceptionType,e.state,ei.id,ei.content,ei.author,ei.create_time,ei.pass_time,ei.answer_count,ei.title FROM sys_exception_info as ei LEFT JOIN sys_exception_exceptionInfo as ee ON ee.eiid=ei.id LEFT JOIN sys_exception as e ON ee.eid=e.id left join sys_user_exception ue on ue.eid=ee.eid left join sys_user_userdetail uu on uu.uid=ue.uid left join sys_user_detail ud on uu.udid=ud.id WHERE e.state=#{state} and e.isFinish=#{param} order by ${orderBy} desc")
    ...
    @Select("select ud.avatar, e.id as eid,e.code as exceptionCode,e.flow_code as flowType,e.exception_type as exceptionType,e.state,ei.id,ei.content,ei.author,ei.create_time ,ei.pass_time,ei.answer_count,ei.title FROM sys_exception_info as ei LEFT JOIN sys_exception_exceptionInfo as ee ON ee.eiid=ei.id LEFT JOIN sys_exception as e ON ee.eid=e.id left join sys_user_exception ue on ue.eid=ee.eid left join sys_user_userdetail uu on uu.uid=ue.uid left join sys_user_detail ud on uu.udid=ud.id WHERE e.state=#{state} order by ${orderBy} desc")
