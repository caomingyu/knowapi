layui.use(['layer', 'form', 'table', 'ztree', 'laydate', 'admin', 'ax', 'util'], function () {
        ...
        MgrException.initColumn = function () {
            return [[
                {type: 'checkbox'},
                {field: 'eid', hide: true, sort: true, title: '异常id'},
                {field: 'id', hide: true, sort: true, title: '异常详细信息id'},
        ...
        MgrException.onDeleteException = function (data) {
            layer.confirm("是否删除异常" + data.exceptionCode + "?", {bet: ["确认", "取消"]},
                function () {
                    debugger;
                    $.ajax({
                        url: "/exception/del",
        ...
        MgrException.showDetail = function (data) {
            layer.open({
                ...
                title: '详细信息查看',
                area: ['300px', '400px'],
                content: '/exception/detail?infoId=' + data.id,
                // end: function () {
                //     table.reload(MgrException.tableId);
                // }
            });
...
// 工具条点击事件
        ...
        form.on('switch(status)', function (obj) {
            ...
            MgrException.changeUserStatus(userId, checked);
        });
